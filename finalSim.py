...
def splitGPSToLatLng(gpsString):
    return list(map(float, gpsString.split(",")))
...
try:
    ...
    while True:
        currentLocationString = gpsCoordinate.get()
        lat, lng = splitGPSToLatLng(currentLocationString)
        ...
        if (gpsd.valid & gps.LATLON_SET) != 0:
            ...
            if ((float(splitGPSToLatLng(targetLocationString)[0][0:-3]) == lat[0:-3]) and (
            (float(splitGPSToLatLng(targetLocationString)[1][0:-3]) == lng[0:-3]))):
                # Location reference need not be changed
                altitude = 30  # in meters
                dest = LocationGlobalRelative(splitGPSToLatLng(targetLocationString)[0],
                                              splitGPSToLatLng(targetLocationString)[1], altitude)
                ...
                targetLocationString = currentLocationString
